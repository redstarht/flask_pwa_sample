...
def create_app():
    ...
    app.config["SECRET_KEY"] = os.urandom(24)  # セッション管理に必須。本番では環境変数などから取得することを推奨

    # Flask-Login設定の追加
    login_manager.init_app(app)
    login_manager.login_view = 'main.login'  # ログインが必要な場合のリダイレクト先をログインページに設定
    login_manager.login_message = 'ログインが必要です。' # ログインが必要な場合に表示されるメッセージ
    login_manager.login_message_category = 'info' # メッセージのカテゴリ

    from .models import User
    @login_manager.user_loader
    def load_user(user_id):
        session = SessionLocal() # リクエストコンテキスト外でも呼ばれる可能性があるため、新しいセッションを作成
